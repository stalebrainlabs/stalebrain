...
- **C — Chunked.** The initial report is one screen: each verdict section shows at most
...
- **A — Actionable.** Every finding line ends in what to do, not just what's wrong.
  Verdict first, evidence second, action third. No finding without a proposed action.
- **L — Low-noise.** No preamble, no "I will now…", no restating the request, no
  apologies. Progress is ticker lines only — at most one per ~10 claims per step.
  Confirmed claims are a count, not a list.
- **M — Meaningful.** Plain words. "Your CLAUDE.md says yarn; the lockfile says pnpm"
...
- `est_tokens(file) = round(bytes / 4)` — bytes = file size on disk (`wc -c`,
