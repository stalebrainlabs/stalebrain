...
[references/memory-sources.md](references/memory-sources.md), which covers Claude Code,
...
`PATH` · `SCRIPT` · `SYMBOL` · `DEP` · `FACT` · `OWNER` · `CONVENTION`, plus `OPINION`
...
```
⚡ stale-brain: CLAUDE.md says "yarn install" but only pnpm-lock.yaml exists, so following reality (pnpm). Run /stale-brain for a full audit.
```
